...
# Chamber whitelist/room-facts text lives in agent_modules.chamber_facts
# (extracted so the num_agents-dependent strings are testable without the
# autogen imports above); _describe_chamber is imported at the top.
...
class CustomAgent(BaseChatAgent):
    ...
    # NOTE: the committed-replay imitation API that lived here (queue +
    # begin_imitation/abort) was removed 2026-08-07 — imitation is now
    # GUIDED: the social-act router delivers the target's action sequence
    # + state via social_returns, and the agent stays in control of every
    # action; adoption is credited in the router, not enacted here.
