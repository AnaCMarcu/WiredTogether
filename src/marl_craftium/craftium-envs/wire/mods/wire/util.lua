...
function wire.get_chamber_for_pos(pos)
    local x, z = pos.x, pos.z
    local N = wire.NUM_AGENTS
    local ch3_x1 = 4 * N

    if x >= wire.CH1.x0 and x <= wire.CH1.x1
       and z >= wire.CH1.z0 and z <= wire.CH1.z1 then
        return "ch1"
    elseif x >= wire.CH2.x0 and x <= wire.CH2.x1
       and z >= wire.CH2.z0 and z <= wire.CH2.z1 then
        return "ch2"
    elseif x >= 0 and x <= ch3_x1
       and z >= wire.CH3_Z0 and z <= wire.CH3_NORTH_WALL_Z then
        if z >= wire.CH3_COMMUNAL_Z0 then
            return "ch3_communal"
        else
            return "ch3_cell"
        end
    elseif x >= wire.CH4.x0 and x <= wire.CH4.x1
       and z >= wire.CH4.z0 and z <= wire.CH4.z1 then
        return "ch4"
    elseif x >= wire.CH5.x0 and x <= wire.CH5.x1
       and z >= wire.CH5.z0 and z <= wire.CH5.z1 then
        return "ch5"
    end
    return "unknown"
end
...
function wire.agent_index(name)
    if wire.DEBUG_SINGLE then return 0 end
    local idx = tonumber(name:match("^agent_?(%d+)$"))
    if idx and idx >= 0 and idx < wire.NUM_AGENTS then
        return idx
    end
    return -1
end
...
function wire.cell_teleport_pos(i)
    return {
        x = wire.cell_x_center(i),
        y = wire.FLOOR_Y + 1,
        z = math.floor((wire.CH3_CELL_Z0 + wire.CH3_CELL_Z1) / 2),
    }
end
...
-- `wire:anvil_pedestal` at FLOOR_Y+1, exactly where an agent
...
function wire.ch1_spawn_pos(i)
    local N = wire.NUM_AGENTS
    if N == 3 and wire.CH1_SPAWNS_3 and wire.CH1_SPAWNS_3[i] then
        return wire.CH1_SPAWNS_3[i]
    end
...
function wire.ch2_fallback_spawn_pos(i)
    local N = wire.NUM_AGENTS
    if N == 3 and wire.CH2_FALLBACK_SPAWNS_3
       and wire.CH2_FALLBACK_SPAWNS_3[i] then
        return wire.CH2_FALLBACK_SPAWNS_3[i]
    end
    local c = wire.CH2
    if wire.TEAM_SCALING then
...
function wire.ch4_fallback_spawn_pos(i)
    local N = wire.NUM_AGENTS
    local c = wire.CH4
    if wire.TEAM_SCALING then
...
function wire.ch5_fallback_spawn_pos(i)
    local N = wire.NUM_AGENTS
    local c = wire.CH5
    if wire.TEAM_SCALING then
...
function wire.safe_set_node(pos, node_def)
    local existing = minetest.get_node(pos).name
    if existing == wire.WALL_NODE then return end
