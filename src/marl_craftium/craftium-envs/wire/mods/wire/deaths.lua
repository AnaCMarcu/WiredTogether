-- deaths.lua: player-death handling for WIRE.
--
...
wire.player_dead = wire.player_dead or {}
...
wire._virtual_hp = wire._virtual_hp or {}
-- Per-agent count of would-have-died events this episode (for metrics/logging).
wire.would_die_count = wire.would_die_count or {}
...
wire.would_die_count_ch4 = wire.would_die_count_ch4 or {}
...
local function _signal_episode_over()
    for _, p in ipairs(minetest.get_connected_players()) do
        if not wire.player_dead[p:get_player_name()] then
...
minetest.register_on_player_hpchange(function(player, hp_change, reason)
    ...
    if wire.agent_index(name) < 0 then return hp_change end  -- non-agent

    local pos     = player:get_pos()
    local chamber = pos and wire.get_chamber_for_pos(pos) or nil
    ...
    local prev = wire._virtual_hp[name] or MAX_HP
    ...
    if vhp <= 0 then
        ...
        if chamber == "ch4" then
            ...
            -- in multi-agent wire — see emit_death_event / state_files.lua).
            wire.emit_death_event("woulddie", name, chamber, WOULD_DIE_PENALTY)
            if craftium and craftium.reward then
                craftium.reward(player, WOULD_DIE_PENALTY)  -- backup (LLM-visibility only)
            end
            local n = (wire.would_die_count[name] or 0) + 1
            wire.would_die_count[name] = n
            -- Ch4 near-deaths tracked separately so M23 can reward a clean run.
            wire.would_die_count_ch4[name] =
                (wire.would_die_count_ch4[name] or 0) + 1
...
minetest.register_on_dieplayer(function(player, reason)
    ...
    -- does NOT reach env.step()'s reward channel in multi-agent wire).
    wire.emit_death_event("death", name, chamber, DEATH_PENALTY)
    if craftium and craftium.reward then
