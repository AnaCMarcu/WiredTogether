...
| Model/env exports | `LLM_MODEL_PATH` (→ mismatch context below), `ST_MODEL_NAME`/`SENTENCE_TRANSFORMERS_HOME` (local MiniLM), `HF_HUB_OFFLINE=1`, `CRAFTIUM_ENV_DIR=$REPO/src/marl_craftium/craftium-envs/five-chambers`, `WIREDTOGETHER_RUNS_ROOT=$REPO/runs`, `PYTHONPATH=$REPO/src` | `_common.sh:181-208` |
...
| wandb | on by default (`WANDB=1`), auth via `~/.netrc`; tags auto = `exp_<name>,seed_<N>` + `WANDB_EXTRA_TAGS`; offline-run dirs auto-`wandb sync`ed post-job | `_common.sh:28-48,104-116,295-302` |
