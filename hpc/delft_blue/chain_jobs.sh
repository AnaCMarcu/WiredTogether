...
#
# Usage (either works):
#   sbatch hpc/delft_blue/chain_jobs.sh          ← uses default NUM_CONT=2
#   bash   hpc/delft_blue/chain_jobs.sh 3        ← 1 first + 3 continuations
#
# Pass NUM_CONT via --export when using sbatch:
#   sbatch --export=ALL,NUM_CONT=4 hpc/delft_blue/chain_jobs.sh
# ─────────────────────────────────────────────────────────────────────────────
#
# Example — 1 first job + 3 continuations (4 × 8h = up to 32 h of compute):
#   bash hpc/delft_blue/chain_jobs.sh 3
#
