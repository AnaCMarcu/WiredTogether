...
def run(_run, _config, _log):
    ...
    # Windows forbids ':' in path components, so format the timestamp
    # without colons. The unique_token is used as a directory name for
    # tb_logs/, so it must be a valid path on all platforms.
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    map_name_safe = (
        str(map_name).replace(":", "_").replace("/", "_").replace("\\", "_")
    )
    unique_token = (
        f"{_config['name']}_seed{_config['seed']}_{map_name_safe}_{timestamp}"
    )

    args.unique_token = unique_token
    if args.use_tensorboard:
        tb_base = getattr(args, "local_results_path", None) or os.path.join(
            dirname(dirname(abspath(__file__))), "results"
        )
        tb_logs_direc = os.path.join(tb_base, "tb_logs")
