...
# smaclite is optional — only needed for the SMAClite env. We don't use
# SMAClite in hebbian-marl, so tolerate its absence.
try:
    from .smaclite_wrapper import SMACliteWrapper
except ImportError:
    SMACliteWrapper = None

# Side-effect import: registers `Foraging-Comm-WxH-Np-Mf-v3` env ids so
# that gymma's `gym.make(key)` can find them. See lbf_comm_wrapper.py.
from . import lbf_comm_wrapper  # noqa: F401
...
def smaclite_fn(**kwargs) -> MultiAgentEnv:
    if SMACliteWrapper is None:
        raise ImportError(
            "smaclite is not installed — `pip install smaclite` to use this env."
        )
