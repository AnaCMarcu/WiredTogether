...
class Logger:
    ...
    def setup_tb(self, directory_name):
        # Switched from the third-party `tensorboard_logger` (dead on modern
        # protobuf; needs tensorflow as a transitive dep) to PyTorch's
        # built-in SummaryWriter — same on-disk event format, no extra deps
        # beyond the `tensorboard` pip package (no tensorflow required).
        from torch.utils.tensorboard import SummaryWriter

        self._tb_writer = SummaryWriter(log_dir=directory_name)
        # Match the (key, value, step) signature the rest of the class expects.
        self.tb_logger = lambda k, v, t: self._tb_writer.add_scalar(k, v, t)
        self.use_tb = True
