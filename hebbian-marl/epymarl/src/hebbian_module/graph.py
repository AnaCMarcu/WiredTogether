...
Ported verbatim (math unchanged) from the parent WiredTogether project.
Only the import path and a few docstring fragments have been adjusted.
"""
...
class HebbianSocialGraph:
    ...
    agent_roles : list[int], optional
        Role index per agent. Used for the modularity_proxy metric.
        For LBF (no role concept) leave as ``None``.
    """
    ...
    def _compute_modulator(
        ...
        # m_ltd into saturation every step.
    ...
    def _update_failure_window(
        ...
        regardless of whether the team is genuinely failing.
        """
        cfg = self.config
    ...
    def update(
        ...
        # Failure-grace LTP bonus (opt-in). When team is failing and the
        # pair's F_ij is still below threshold, add LTP proportional to
        # co-activity — the "we tried something together" reinforcement.
        # The bonus fades to zero once F_ij ≥ failure_grace_threshold,
        # after which only the existing sustained LTD applies, so the
        # bond's per-step delta flips from positive to negative across
        # the threshold. Disabled by default — legacy hebb_* variants
        # never reach this branch and get bit-identical math.
        grace_bonus = None
    ...
    # ──────────────────────────────────────────────
    # 2.7  Social replay (Eq. 7) — see notes in config.py
    # ──────────────────────────────────────────────
    ...
    def get_graph_metrics(self) -> Dict:
        ...
        without losing pairs that drop out of the top-3 mid-episode.
        """
        if not self.config.enabled:
            return {}
    ...
    def snapshot(self) -> dict:
        """Compact per-step snapshot for bonds.jsonl."""
        d = self.to_dict()
        return {
            "W": d.get("W"),
