...
FS = dict(axis=11, tick=10, panel_title=11.5, caption=8.0, body=8.2,
          header=8.6, raster=8.0, chamber=8.5)
PANEL_EDGE = "#c3c9d0"
...
RC = {"font.family": "serif",
      ...
      "axes.edgecolor": "#9aa3ad", "svg.fonttype": "none",
...
ARMS = {
    "gemma3f_seed123": dict(
        ...
        examples=[
            dict(key="anvil", x=(2099, 2125), ep=3, agents=(1, 0),
                 dpairs=[], title="Failure in coordination",
                 frames=[("F", (1, 99, ""), (0, 101, "")),
            ...
            dict(key="kill", x=(710, 730), ep=1, agents=(1, 0),
                 dpairs=[(1, 0)], title="Partial coordination in combat",
            ...
            dict(key="switch", x=(426, 450), ep=1, agents=(0, 1),
                 dpairs=[(0, 1)], title="Coordination on request",
        ...
        figures=[
            ...
            dict(name="overview_2panel", xlim=None, keys=["switch", "anvil"],
                 delib=False),
    ...
    "orch789": dict(
        ...
        examples=[
            dict(key="kill", x=(683, 696), ep=1, agents=(2, 1), dpairs=[],
                 title="Coordination via the orchestrator",
        ...
        figures=[
            dict(name="overview", xlim=None, keys=None, delib=False),
            dict(name="overview_2panel", xlim=None, keys=["switch", "solokill"],
                 delib=False),
    ...
    "qwen9b_base789": dict(
        ...
        figures=[
            dict(name="overview", xlim=None, keys=None, delib=False),
            dict(name="overview_2panel", xlim=None, keys=["anvil", "wipe"],
                 delib=False),
        ],
        filmstrips=[
    ...
    "qwen9b_heb42": dict(
        ...
        figures=[
            dict(name="overview", xlim=None, keys=None, delib=False),
            dict(name="overview_2panel", xlim=None, keys=["anvil", "phantom"],
                 delib=False),
        ],
        filmstrips=[
...
def lane_bonds(ax, arm, Wr, spans, xlim):
    ...
    for t, q in arm["joints"]:
        ...
        ax.scatter(t, y, marker="o", s=58, facecolor="white",
                   edgecolor=PAIR_C[q], linewidths=1.8, zorder=6)
    for t, killer in arm["kills"]:
        ys = [(Wr[t, q[0], q[1]] + Wr[t, q[1], q[0]]) / 2
              for q in PAIR_C if killer in q]
        ax.scatter(t, max(ys), marker="*", s=160, facecolor=AGENT_C[killer],
    ...
    leg1 = ax.legend(fontsize=9, ncol=3, frameon=False, loc="upper left",
                     borderaxespad=0.2)
    ax.add_artist(leg1)
    ax.legend(handles=_event_handles(), fontsize=8, ncol=2, frameon=False,
              loc="upper right", borderaxespad=0.2, handletextpad=0.4)
    ax.set_ylim(0.05, ymax * 1.28)
...
def lane_coassign(ax, arm, run, spans, xlim):
    ...
    bars = [Patch(facecolor="#7b838d", alpha=0.95,
                  label="pair on the same task"),
            Patch(facecolor="#7b838d", alpha=0.28,
                  label="both assigned, different tasks")]
    leg1 = ax.legend(handles=bars, fontsize=8, ncol=2, frameon=False,
...
def _event_handles(kill_label="mob kill (agent colour)"):
    from matplotlib.lines import Line2D
    return [Line2D([], [], marker="o", ls="none", mfc="white",
                   mec=PAIR_C[(0, 1)], mew=1.6, ms=7,
                   label="joint milestone (both agents credited)"),
            Line2D([], [], marker="*", ls="none", color=MUTED, ms=12,
                   label=kill_label)]
...
def panel_content_h(ex, w, W_in, H_in, pad=0.006):
    """Figure-fraction height one panel actually needs."""
    import textwrap
    colw = (w - 3 * pad) / 2
    fr_h = (colw * W_in * 9 / 16) / H_in
    h = 0.018 + 0.024 + 0.010
    for row in ex["frames"]:
        if row[0] == "F":
            h += fr_h + 0.008
        else:
            h += 0.0150 + 0.0125 * (len(textwrap.wrap(row[4], 46)) - 1) + 0.0032
    return h + 0.012 + 0.020
...
def panel(fig, arm, ex, frames, Wr, x0, w, y0, h, show_delib):
    ...
    colw_pre = (w - 3 * pad) / 2
    fr_h = (colw_pre * W_in * 9 / 16) / H_in     # exact 16:9, max width
    cap_h, msg_h = 0.008, 0.0150
    for row in ex["frames"]:
        if row[0] == "F":
            y -= fr_h
...
def build_figure(arm_name, arm, spec, run, Wr, messages, spans, frames,
    ...
    ph = arm.get("ph", 0.60) + (0.10 if show_delib else 0.0)
    fig_w = {1: 6.2, 2: 9.6}.get(len(examples), 13.6)
    fig = plt.figure(figsize=(fig_w, 10.6 * (0.42 + ph) / 1.02
                              + (1.2 if show_delib else 0.0)))
    bottom = ph + 0.02 + 0.045
    two_lanes = arm["lane"] in ("bonds_replay", "bonds_snap", "coassign")
    if two_lanes:
        gs = fig.add_gridspec(2, 1, height_ratios=[0.38, 1.0], hspace=0.08,
                              left=0.065, right=0.985, top=0.975,
                              bottom=bottom)
        axR = fig.add_subplot(gs[0])
        axB = fig.add_subplot(gs[1], sharex=axR)
    else:
        gs = fig.add_gridspec(1, 1, left=0.065, right=0.985, top=0.965,
                              bottom=bottom)
    ...
    if arm["lane"] in ("bonds_replay", "bonds_snap"):
        ...
        for t, q in arm["joints"]:
            ax = axB
            ax.scatter(t, y_of[q] + 0.5, marker="o", s=58,
                       facecolor="white", edgecolor=PAIR_C[q],
                       linewidths=1.8, zorder=6)
        for t, killer in arm["kills"]:
            axB.scatter(t, 3.25, marker="*", s=160,
                        facecolor=AGENT_C[killer], edgecolor="white",
                        linewidths=0.6, zorder=6, clip_on=False)
        chamber_labels(axB, spans, -0.36, xlim)
    axB.set_xlabel("environment step (cumulative)", fontsize=FS["axis"],
                   labelpad=2)
    axB.tick_params(labelsize=FS["tick"])

    boxes = []
    span = xlim[1] - xlim[0]
    for ex in examples:
        lo, hi = ex["x"]
        pad_x = max((hi - lo) * 0.75, span * 0.006)
        lo, hi = lo - pad_x, hi + pad_x
        axB.add_patch(Rectangle((lo, 0), hi - lo, 1,
                                transform=axB.get_xaxis_transform(),
                                facecolor="#3f4650", alpha=0.16,
                                edgecolor="#2b3038", lw=1.3, zorder=5))
        if axR is not None:
            axR.axvspan(lo, hi, color="#3f4650", alpha=0.13, zorder=1,
                        linewidth=0)
        boxes.append((lo, hi))
    for ax in filter(None, (axR, axB)):
        for e, (o, _) in enumerate(run["_ep_bounds"]):
            if e and xlim[0] < o < xlim[1]:
                ax.axvline(o, color=INK, lw=0.9, ls=(0, (4, 3)), alpha=0.35)
    ...
    n = len(examples)
    cw = min(0.46, (0.95 - 0.010 * (n - 1)) / n)
    gap = 0.010
    left = 0.5 - (n * cw + (n - 1) * gap) / 2
    W_in, H_in = fig.get_size_inches()
    ph_fit = min(ph, max(panel_content_h(ex, cw, W_in, H_in)
                         for ex in examples))
    for k, (ex, box) in enumerate(zip(examples, boxes)):
        x0 = left + k * (cw + gap)
        panel(fig, arm, ex, frames, Wr, x0, cw, 0.005 + (ph - ph_fit),
              ph_fit, show_delib)
        tr = axB.get_xaxis_transform()
        for x_data, x_fig in ((box[0], x0), (box[1], x0 + cw)):
            fig.add_artist(ConnectionPatch(
                xyA=(x_data, 0.0), coordsA=tr,
                xyB=(x_fig, 0.005 + ph), coordsB=fig.transFigure,
