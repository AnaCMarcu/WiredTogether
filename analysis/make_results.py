...
Usage:
  python analysis/make_results.py --runs-root runs/final --out paper_assets
  python analysis/make_results.py --runs-root runs/final --out paper_assets \
...
CONDITIONS = [
    ...
    # Social-replay (Eq. 7 experience sharing) arms, Qwen lane. Same medium
    # config as exp05/exp06 (3 eps x 1000 steps, Qwen3.5-2B); skipped
    # automatically when the run dirs are absent.
    ("MAPPO+Heb+SR", "exp30_mappo_hebbian_replay", "main", True),
    ("IPPO+Heb+SR",  "exp31_ippo_hebbian_replay",  "main", True),
    ...
    # Orchestrator baselines (runs/orchestrator, submit_orchestrator.sh).
    # Group "orchestrator": no table emitter selects it (same pattern as
    # "gemma" / "pareto" / "scaling"), present so the qualitative registry
    # can discover these runs for the perception axis. Not a Hebbian arm --
    # the coupling is a centralized coordinator, so hebbian? is False.
