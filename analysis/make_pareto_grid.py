...
tables — the helpers are IMPORTED from it, not re-implemented):
    ...
    flops          inference compute 2*N_eff*(prefill+decode tokens), summed over
                   all agents/modules/retries (analysis/compute_flops.py); shown
...
from paths import ASSETS, QUALITATIVE, REPO, RUNS  # noqa: E402  (also puts siblings on sys.path)
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=ASSETS / "pareto" / "grid")
