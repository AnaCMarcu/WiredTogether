...
the lower left, the social figure's y-headroom rule. Axis names follow the
...
# Models whose label hangs below the lower arm. Empty on both axes: the
# legend sits in the lower left, where the E2B label used to hang.
...
def draw(rows, out_png: Path, xlabel: str, errorbars: str = "none",
         label_below=frozenset(), legend_loc="lower left"):
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=ASSETS / "pareto" / "paper")
    ap.add_argument("--beliefs", type=Path, nargs="+",
                    default=[Path("analysis/qualitative/out_gemma/tables/beliefs.csv"),
                             Path("analysis/qualitative/out_pareto/tables/beliefs.csv"),
                             Path("analysis/qualitative/out/tables/beliefs.csv")])
    ...
    ap.add_argument("--legend-loc", default="lower left",
                    help="matplotlib legend location (both axes: lower left)")
