...
Importing this module makes ``analysis/`` (for ``make_results``) and ``src/``
...
QUALITATIVE = Path(__file__).resolve().parents[1]
ANALYSIS = QUALITATIVE.parent
REPO = ANALYSIS.parent
SRC = REPO / "src"
for _p in (str(SRC), str(REPO), str(ANALYSIS), str(QUALITATIVE)):
...
import make_results  # noqa: E402  (analysis/; import-safe, __main__-guarded)
