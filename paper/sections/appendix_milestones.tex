...
Two negative reward components are applied outside the milestone schedule. The first is a death penalty, applied at two magnitudes. A real death in the lethal boss chamber (Chamber~5), where a lethal hit ends the agent's episode, incurs $-50$ points. In the forgiving chambers (Chambers~1--4) the agent is invincible and takes no real damage, but accumulated would-be damage is tracked in a virtual-health pool, and each time that pool reaches a lethal threshold a smaller $-10$ \emph{would-have-died} penalty is recorded (and the pool refills) without any actual death or respawn --- a graded signal that a near-fatal beating is costly but, unlike a real death, not terminal. The second component is a small futile-action penalty of $-1$ point, applied when an action is structurally wasted --- for example a camera tilt past the pitch cap that the engine cannot apply --- giving the policy a per-action validity signal that the team-level milestone reward lacks.
...
M23 & ch4\_combat   & All agents survive Ch4 with no would-have-died event & 100  & T \\
\midrule
